...
int main(int argc, char **argv)
{
	...
	for (ptr = sock_opts; ptr->opt_str != NULL; ptr++) 
	{
		...
		else 
		{
			...
			len = sizeof(val);
			//调用getsockopt，不受支持的选项应该引发一个ENOPROTOOPT错误
			...
			{
				//返回成功，则调用相应的选项值输出函数将选项值转换为一个字符串并输出
...
{
/* *INDENT-OFF* */
	//因为getsockopt的最后一个参数时值-结果参数，所以做的第一项检查就是getsockopt返回值的大小是否为期望的大小
	if (len != sizeof(int))

...
static void do_funccall(const char *host, const char *serv, int flags, int family, int socktype, int protocol, int exprc)
{
	...
	/*getaddrinfo函数能够处理名字到地址以及服务到端口这两种转换，返回的时一个sockaddr结构而不是一个地址列表，这些sockaddr结构随后可由套接字函数直接使用
	/第一个参数是一个主机名或地址串（IPv4的点分十进制数串或Ipv6的十六禁止数串）
	/第二个参数是一个服务名或十进制端口号数串
	/第三个参数可以是一个空指针，也可以时指向某个addrinfo结构的指针，调用者在这个结构中填入关于期望返回的信息类型的暗示。
	如果hints参数时一个空指针，本函数就假设ai_flag、ai_socktype和ai_protocol的值均为0，ai_family的值为AF_UNSPEC（地址族未指定）
	/如果本函数返回成功（0），那么由最后一个参数result指向的变量已被填入一个指针，它指向的时由其中的ai_next成员串接起来的addrinfo结构链表
	/导致返回多个addrinfo结构的情形由一下两个：
	（1）如果与hostname参数关联的地址有多个，那么适用于所有请求地址族（可通过hints结构的ai_family成员设置）的每个地址都返回一个对应的结构。
...
static int do_onetest(char *host, char *serv, struct addrinfo *hints, int iteration)
{
	...
	do {
		//链表头
		if (iteration == 1) 
		{	/* always print results first time */

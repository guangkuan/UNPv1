...
void pr_ipv4(char **listptr)
{
	...
	for ( ; *listptr != NULL; listptr++) 
	{
		...
		/*gethostbyaddr的可重入版本
		不可重入的原因：它们都返回指向同一个静态结构的指针
		*/
		if (gethostbyaddr_r((char *) &inaddr, sizeof(struct in_addr), AF_INET, &hent, buf, sizeof(buf), &res, &h_errno) == 0)
			printf("    (gethostbyaddr failed: %s)\n", hstrerror(h_errno));
		else if (res->h_name != NULL)
			printf("    name = %s\n", res->h_name);
		else
